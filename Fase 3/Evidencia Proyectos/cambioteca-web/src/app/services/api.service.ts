...
export class ApiService {
  ...
  fetchBookByIsbn(isbn: string): Observable<any> {
    const cleanIsbn = isbn.replace(/[^0-9X]/gi, '');
    const url = `https://www.googleapis.com/books/v1/volumes?q=isbn:${cleanIsbn}`;
    
    // Seguridad extra: Si por alguna razón el constructor falló, lo creamos aquí mismo
    if (!this.externalHttpClient) {
      this.externalHttpClient = new HttpClient(this.handler);
    }

    return this.externalHttpClient.get(url);
  }
